#!/bin/bash
# vars.sh — shared environment for all GoW plugin scripts

export GOW_NAME="gow"
export GOW_VERSION="2026.05.30"
export GOW_ORG="games-on-whales"
export GOW_PLUGIN="/boot/config/plugins/gow"
export GOW_CFG="${GOW_PLUGIN}/gow.cfg"
export GOW_SCRIPT_DIR="${GOW_PLUGIN}/scripts"
export GOW_EMHTTP="/usr/local/emhttp/plugins/gow"
export GOW_PACKAGE_DIR="${GOW_PLUGIN}/packages"
export DEFAULT_APPDATA="/mnt/user/appdata/gow"
export DEFAULT_STEAM_LIBRARY="/mnt/user/steam"
export DEFAULT_GAMES_LIBRARY="/mnt/user/games"
export DEFAULT_ROMS_LIBRARY="/mnt/user/roms"
export DEFAULT_BIOS_LIBRARY="/mnt/user/bioses"
export DEFAULT_MEDIA_LIBRARY="/mnt/user/media"
export DEFAULT_LUTRIS_LIBRARY="${DEFAULT_APPDATA}/lutris"
export DEFAULT_COMPAT_TOOLS_PATH="${DEFAULT_APPDATA}/compatibilitytools.d"
