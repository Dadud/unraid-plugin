...
# Keep in sync with <FILE Name="&script_dir;/..."> entries in gow.plg
SCRIPT_NAMES=(
  vars.sh preinstall.sh install.sh deploy.sh uninstall.sh update.sh reset.sh
  wipe-full.sh library-links.sh apply-mount-presets.sh apply-mount-presets.py
  pairing-state.sh detect-paths.sh health-check.sh fix-all.sh
  cleanup-wolf-sessions.sh apply-ui.sh hotfix-page.sh dev-sync.sh library-audit.sh
)
